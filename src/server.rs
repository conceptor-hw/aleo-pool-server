        ...
        match msg {
            ...
            ServerMessage::NewBlockTemplate(block_template) => {
                ...
                for (peer_addr, sender) in self.authenticated_provers.read().await.clone().iter() {
                    ...
                    redis_publisher::publish_message(message::PUB_BINARY_CHANNEL, ProverMessage::Notify(block_template.clone(), u64::MAX / difficulty)
                    ).unwrap();
                }
            }
            ServerMessage::ProverSubmit(peer_addr, block_height, nonce, proof) => {
                ...
                task::spawn(async move {
                    async fn send_result(sender: &Sender<ProverMessage>, result: bool, desc: Option<String>) {
                        if let Err(e) = sender.send(ProverMessage::SubmitResult(result, desc.clone())).await {
                            error!("Error sending result to prover: {}", e);
                        }
                        redis_publisher::publish_message(message::PUB_BINARY_CHANNEL, ProverMessage::SubmitResult(result, desc)).unwrap();
                    }
